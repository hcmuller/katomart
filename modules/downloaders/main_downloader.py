...
class Downloader:
    ...
    def get_video_playlist(self):
        ...
        if len(data['media_assets']) > 0:
            highest_quality_asset = max(data['media_assets'], key=lambda x: x['height'])
            playlist_url = highest_quality_asset.get('url')
            ...
            self.download_ytdlp_media(playlist_url, referer=self.current_content_url, save_path=self.download_path, media_index=self.file_name.split('.', 1)[0])
